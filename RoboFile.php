...
class RoboFile extends Tasks implements LoggerAwareInterface, ConfigAwareInterface
{
    ...
    public function cmdReleaseBuildExecute(
        string $destination = './artifacts/coverage-merger.phar',
        array $options = [
            'tag' => '',
        ]
    ) {
    ...
    protected function getTaskReleaseBuildPhar(string $pharPathname, string $version): \Closure
    {
        return function () use ($pharPathname, $version): int {
            ...
            if (file_exists($pharPathname)) {
                $this->logger->info(
                    'delete previous release: {fileName}',
                    [
                        'fileName' => $pharPathname,
                    ],
                );
                unlink($pharPathname);
            }

            $appName = preg_replace('/-cli$/u', '', $this->packageName);
            $startFile = "bin/$appName";
            $startContent = file($startFile);
            array_shift($startContent);
            if ($version !== '') {
                $this->logger->info(
                    'replace version in statContent with {version}',
                    [
                        'version' => $version,
                    ],
                );
            ...
            $this->logger->info(
                'create release destination directory {dir}',
                [
                    'dir' => Path::getDirectory($pharPathname),
                ],
            );
            $this->fs->mkdir(Path::getDirectory($pharPathname), 0777 - umask());

            $this->logger->info(
                'build Phar file {fileName}',
                [
                    'fileName' => $pharPathname,
                ],
            );
...
PHP,
     ...
     */
    public function cmdPharContentExecute(string $path = './artifacts/coverage-merger.phar')
    {
        $path = realpath($path);
